...
class Chatwoot_Client {
  ...
  async getAttributes(userPhone) {
    ...
    const contact = await data.payload[0];
  ...
  async getConversationID(userID) {
    ...
    const conversation = conversations.find(
      (c) => c.inbox_id == (this.CHATWOOT_INBOX_ID)
    );
    if (!conversation) {
      throw new Error(
   ...
   */
  async ValidateUser(userPhone, name) {
   ...
   */
  async handleMediaData(MediaData, form) {
    try {
  ...
  extractFileName(url, contentType) {
